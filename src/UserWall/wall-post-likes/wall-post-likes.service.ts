...
export class WallPostLikesService {
  ...
  constructor(@InjectRepository(WallPostLike) private likesRepo: Repository<WallPostLike>) {}

  create(postId: number, userId: number) {
    return this.likesRepo.save({ post: { id: postId }, user: { id: userId }, filterUserId: userId })
  }
