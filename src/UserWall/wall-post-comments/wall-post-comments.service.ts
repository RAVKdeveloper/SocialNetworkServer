...
export class WallPostCommentsService {
  constructor(
    ...
    private commentsRepo: Repository<WallPostComment>,
  ) {}
  ...
  uploadImage(userId: number, fileName: string, commentsId: number) {
    return this.commentsRepo.update(
      { id: commentsId, user: { id: userId } },
      { imgUrl: fileName },
    );
  }
  ...
  async remove(id: number, userId: number) {
    const { id: commentId, imgUrl } = await this.commentsRepo.findOne({
  ...
  async deleteImage(fileName: string) {
    ...
    const file = path.join(
      __dirname,
      `../../../uploads/WallMedia/comments/${fileName}`,
    );
