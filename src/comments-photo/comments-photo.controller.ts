...
export class CommentsPhotoController {
  ...
  @Post()
  create(
    @Body() createCommentsPhotoDto: CreateCommentsPhotoDto,
    @Request() request,
  ) {
    return this.commentsPhotoService.create(
      createCommentsPhotoDto,
      request.user.sub,
    );
  }
  ...
  update(
    @Param('id') id: string,
    @Body() updateCommentsPhotoDto: UpdateCommentsPhotoDto,
  ) {
