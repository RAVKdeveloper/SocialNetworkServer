...
export class GaleryClipsLikesService {
  ...
  constructor(@InjectRepository(GaleryClipsLike) private likesRepo: Repository<GaleryClipsLike>) {}
  ...
  async findAll(clipId: number, userId: number) {
      const likes = await this.likesRepo.find({ relations: {
           user: true,
           clip: true
  ...
  async remove(id: number, userId: number) {
      const like = await this.likesRepo.findOne({ relations: {
        user: true,
        clip: true
